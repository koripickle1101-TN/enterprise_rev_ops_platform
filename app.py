...
def split_items(value): return [item.strip() for item in str(value or "").split(";") if item.strip()]
...
def readiness_score(case):
    missing = len(missing_docs(case)); overdue = max(0, int(case.get("days", 0)) - int(case.get("sla", 5)))
...
def wlcm_scores(case):
    scores = {domain: 86 for domain in CONTROL_DOMAINS}; loss = case.get("loss", "None")
    if loss in scores: scores[loss] -= 32
    if case.get("domain") in ["Patient Access", "Eligibility Verification", "Financial Clearance"]: scores["Access Control"] -= 12
    scores["Documentation Control"] -= len(missing_docs(case)) * 9
    if case.get("domain") == "Authorization Control": scores["Authorization Control"] -= 16
...
def stability_status(case):
    score = readiness_score(case); overdue = max(0, int(case.get("days", 0)) - int(case.get("sla", 5)))
    if case.get("risk") == "High" and overdue >= 2: return "Escalation Required"
    if score < 55: return "Control Loss"
    if score < 78: return "Watch"
    return "Stable"
def auth_readiness(case):
    present = split_items(case.get("present", ""))
    factors = {"Eligibility verified": any(x in present for x in ["Eligibility Response", "Benefit Check"]), "Benefit checked": "Benefit Check" in present, "Service requirement reviewed": any(x in present for x in ["Procedure Code", "Site of Service"]), "Clinical note present": any(x in present for x in ["Clinical Note", "Cardiology Note", "Neuro Exam", "Surgical Note", "Assessment"]), "Medical necessity support present": "Medical Necessity Note" in present, "Prior therapy history present when required": any(x in present for x in ["Prior Therapy History", "Failed Conservative Therapy", "Failed Therapy Evidence"]) or "Therapy" not in case.get("required", ""), "Payer route confirmed": "Payer Route" in present or case.get("domain") != "Routing Intelligence", "Submission owner assigned": bool(case.get("owner", "")), "Follow-up window documented": int(case.get("days", 0)) <= int(case.get("sla", 5))}
    met = sum(1 for value in factors.values() if value); score = round(met / len(factors) * 100, 1)
...
def sla_status(case):
    ...
    if remaining < 0: return remaining, "Breached", "Escalate today"
...
def parse_csv(uploaded):
    ...
    try:
        raw = uploaded.getvalue().decode("utf-8"); reader = csv.DictReader(StringIO(raw)); rows = []
        for row in reader:
            ...
            for key in ["days", "sla", "exposure"]:
                try: item[key] = int(float(item[key]))
                except Exception: item[key] = 0
            if item.get("id"): rows.append(item)
    ...
    except Exception: return DEFAULT_DATA, "CSV could not be read. Default synthetic data active."
def summary(rows):
    total = len(rows); high = sum(1 for row in rows if row.get("risk") == "High"); sla = sum(1 for row in rows if int(row.get("days", 0)) >= int(row.get("sla", 5)))
    exposure = sum(int(row.get("exposure", 0)) for row in rows); owners = len(set(row.get("owner", "") for row in rows if row.get("owner", "")))
...
def most_common(rows, key):
    values = [row.get(key, "") for row in rows if row.get(key, "")]; return Counter(values).most_common(1)[0][0] if values else "None"
def common_missing(rows):
    docs = []
    for row in rows: docs.extend(missing_docs(row))
...
.case-title{{font-family:'Inter',sans-serif;font-size:clamp(2.55rem,4.7vw,4rem);line-height:1;letter-spacing:-.055em;font-weight:300;font-variant-numeric:tabular-nums;margin-bottom:1.05rem;white-space:nowrap;}}
...
.bar-number{{font-family:'Inter',sans-serif;font-size:2rem;color:var(--orange);font-weight:300;font-variant-numeric:tabular-nums;line-height:1;}}
...
.stButton>button,.stDownloadButton>button{{background:var(--orange)!important;color:var(--black)!important;border:1.5px solid var(--black)!important;border-left:14px solid var(--black)!important;border-radius:0!important;font-weight:700!important;letter-spacing:.12em!important;text-transform:uppercase!important;}}
...
all_risks = sorted({row["risk"] for row in rows}); all_payers = sorted({row["payer"] for row in rows}); all_domains = sorted({row["domain"] for row in rows}); all_lines = sorted({row["line"] for row in rows})
with st.expander("Command Filters", expanded=False):
    c1, c2 = st.columns(2)
    with c1:
        selected_risks = st.multiselect("Filter by risk level", all_risks, default=all_risks); selected_domains = st.multiselect("Filter by workflow area", all_domains, default=all_domains)
    with c2:
        selected_payers = st.multiselect("Filter by payer group", all_payers, default=all_payers); selected_lines = st.multiselect("Filter by service line", all_lines, default=all_lines)
...
if not filtered_rows: filtered_rows = rows
...
if module == "Command Center":
    ...
    risk_counts = Counter(row["risk"] for row in filtered_rows); max_risk = max(risk_counts.values()) if risk_counts else 1
    ...
    if note: st.success("Human review note captured for this session. Do not enter PHI.")
    ...
    factors, score, status = auth_readiness(selected_case); rows_html = ''.join(f'<div class="case-row"><strong>{esc(name)}:</strong> {"Met" if value else "Gap"}</div>' for name, value in factors.items())
    ...
    missing = missing_docs(selected_case); render_html('<div class="module-title">Missing Documentation Detector</div>')
    if missing: render_html('<div class="section-card"><div class="eyebrow">Detected Documentation Gaps</div><div class="chip-wrap">' + ''.join(chip(item) for item in missing) + f'</div><div class="lead">Operational impact: {esc(selected_case["rule"])}</div><div class="case-row"><strong>Next action:</strong> {esc(selected_case["action"])}</div><div class="case-row"><strong>Owner:</strong> {esc(selected_case["owner"])}</div></div>')
    else: render_html('<div class="section-card"><div class="identity-copy">No missing documentation detected for this synthetic case.</div></div>')
    ...
    reviewer = st.text_input("Reviewer name", value="Kori Pickle"); action_taken = st.selectbox("Action taken", ["Documentation request", "Eligibility follow-up", "Payer route correction", "Authorization escalation", "Ready for submission", "Leadership escalation"]); decision_status = st.selectbox("Decision status", ["Open", "In Review", "Escalated", "Completed"]); notes = st.text_area("Review notes", placeholder="Use synthetic details only. Do not enter PHI."); follow_date = st.date_input("Follow-up date")
    ...
    brief = executive_brief(filtered_rows); render_html('<div class="module-title">Executive Brief Builder</div>'); st.text_area("Leadership brief generated from the active command view", brief, height=420); st.download_button("Download Executive Brief", brief, file_name="kori_pickle_executive_brief.txt")
    ...
    closure_gain = st.slider("Documentation gap closure target", 0, 100, 45); follow_gain = st.slider("Follow-up reliability improvement", 0, 100, 35); new_score = min(100, round(avg_ready + closure_gain * 0.28 + follow_gain * 0.18, 1)); avoided = int(exposure * (new_score - avg_ready) / 100)
